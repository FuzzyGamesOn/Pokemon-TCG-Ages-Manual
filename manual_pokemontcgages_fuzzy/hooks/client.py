...
def customize_client(ctx: ManualContext):
    # add a custom tab to the client named "Deck Builder"
    ctx.ui.logging_pairs.append(
        ("Deck Builder", "Deck Builder")
    )

    # backup the normal Manual on_package so we can add our own after the normal one
    ctx.on_package_default = ctx.on_package
    ctx.on_package = functools.partial(on_package, ctx)

    ctx.last_deck_contents = []
    ctx.enemies = {}

def on_package(ctx: ManualContext, cmd: str, args: dict):
    ctx.on_package_default(cmd, args)
    
    if cmd in {"Connected"}:
        ctx.enemies = json.loads(args['slot_data'].get('enemies', "{}"))

        update_custom_ui(ctx)
    elif cmd in {"DataPackage", "ReceivedItems", "RoomUpdate"}:
        update_custom_ui(ctx)

def update_custom_ui(ctx: ManualContext):
        ...
        for _, child in enumerate(deck_contents.children):
            for _, card_child in enumerate(child.children):
                if type(card_child) is Label:
                    ctx.last_deck_contents.append(card_child.text)
    ...
    unlocked_cards = [
        ctx.item_names[network_item.item] for network_item in ctx.items_received
    ]
    ...
    enemy_cards = [
        f"{enemy_hp} HP: {enemy_card}" for enemy_hp, enemy_card in ctx.enemies.items()
    ]
    ...
    enemy_dropdown_btn = Button(text="Copy Enemy Decklist", size_hint=(None, None), size=(dp(150), dp(30)))
    ...
    for child in ctx.ui.tabs.tab_list:
    ...
    if len(ctx.last_deck_contents) > 0:
        for deck_listing in ctx.last_deck_contents.reverse():
...
async def main(args):
    ...
    if gui_enabled:
        ctx.run_gui()
        ctx.game = game_name # set the game name to change the Manual Game ID field
        ctx.ui.base_title = client_name # set a custom title for the custom client window
        customize_client(ctx) # this triggers our other customizations to the client window
